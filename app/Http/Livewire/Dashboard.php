...
class Dashboard extends Component
{
    ...
    public function mount()
    {
        ...
        {
            $this->my_curses = CurseLog::with('leaderboard_recieved')
                            ->where('reciever_hash', Auth::user()->player_hash)
                            ->where('type', 'curse')
                            ->orderBy('character_id', 'desc')
                            ->limit(10)
                            ->get();

            $this->my_trusts = CurseLog::with('leaderboard_recieved')
                            ->where('reciever_hash', Auth::user()->player_hash)
                            ->where('type', 'trust')
                            ->orderBy('character_id', 'desc')
                            ->limit(10)
    ...
    public function render()
    {
        ...
        $this->yumlogs = Yumlog::where('user_id', Auth::user()->id)
                        ...
                        ->where('status', '!=', 2)
                        ->select('timestamp', 'character_id', 'character_name', 'curse_name', 'player_hash', 'verified', 'created_at')
                        ->groupBy('curse_name')
    ...
    public function verifyCurses()
    {
        ...
        $this->emitTo('dashboard', 'verificationComplete');
    ...
    public function deleteYumlogEntry($id)
    {
        ...
        $this->emitTo('dashboard', 'logEntryDeleted');
    }
    ...
    public function deleteAllYumlogEntries()
    {
        ...
        $this->emitTo('dashboard', 'logEntriesDeleted');
    }
}
